...
| FR-EXPORT-06 | 中文支持 | P1 | 宋体 11号字体，工作表和表头均支持中文 |
...
| 列 | 内容 | 宽度 | 对齐 |
|----|------|------|------|
| 时间 | YYYY/M/D HH:MM:SS | 135px | 左 |
| 运行时间(秒) | 整数 | auto (header) | 左 |
...
| 元素 | 样式 |
|------|------|
| 表头行 | 加粗、居中（水平+垂直）、宋体 11pt |
| 日期列（时间戳） | Excel 日期序列号，格式 `yyyy/m/d h:mm:ss`、居中 |
| 整数列（运行时间/进程ID） | 数值格式 `0`、居中 |
| 小数数列（CPU/内存/网络） | 数值格式 `0.00`、居中 |
...
| V2.7 | 2026-07 | 日志时间显示格式优化（yyyy/M/d HH:mm:ss）、Excel 导出字体改为宋体 11号、数据类型从文本改为日期/数字格式、单元格居中 |
| V2.8 | 2026-07 | 运行时间改为整数显示、Excel 整数列使用 0 格式、进程ID 在 Excel 中改为整数、文档更新 |
