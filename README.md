...
| **Excel 导出** | 监测中每 10 秒增量写入（防崩溃丢失），停止时从磁盘加载全量数据完成最终导出，系统+进程分 Sheet，超 5 万行自动 PartN |
| **HTML 报告** | Canvas 2D 自绘折线图，X 轴框选缩放、点击显示坐标值、图表过滤器，显示监测时长，自包含无需联网 |
| **配置持久化** | JSON 配置文件（UTF-8），所有设置完整保存，启动自动加载 |
| **帮助系统** | 5 页签帮助对话框（软件简介 / 功能说明 / 使用方法 / 使用窍门 / 注意事项） |
| **窗口置顶** | 按钮切换，方便实时观察 |
| **彩色状态** | 绿色=就绪 / 橙色=监测中 / 红色=异常 |
| **右键菜单** | 数据列表右键：清除日志 / 全选 / 复制选中（Tab 分隔，可粘贴到 Excel） |
| **数据容量** | 内存保留 ~5 分钟热数据，完整历史写入磁盘隐藏临时文件，停止时自动清理 |
| **进程网速缩放** | 按选中网卡占全网总流量比例缩放，选未连接网卡时进程网速为 0 |
| **内存管理** | 导出后主动归还内存给 OS（`shrink_to_fit`），长时运行不累积 |
| **稳定退出** | 关闭窗口完全退出，线程超时强制终止，不留后台残留进程 |
...
- **实时写入**：`BeginExport` 锁定 → 每 10~30s `FlushExport` 增量写入 → `EndExport` 释放
...
6. 点击「开始监测」
7. 点击「停止监测」自动导出 Excel + HTML 报告

### 数据保存规则

- 监测中每 10 秒增量刷新 Excel（防崩溃丢失）
- HTML 趋势报告每 2 分钟自动重新生成
- 内存保留 ~5 分钟热数据，完整历史写入磁盘隐藏临时文件
- 停止时从磁盘加载全量完成最终导出，自动清理临时文件
- 磁盘文件超 5 万行自动分割 `_PartN`，防单文件过大
- 文件名格式：`monitor_data_YYYYMMDDHHmmss.xlsx` / `.html`
