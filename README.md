...
| **进程资源监测** | 按进程名添加，独立 Tab 页展示 PID / CPU / 内存。自动检测进程存活并重新绑定 PID |
| **网卡选择** | 与「控制面板 → 网络连接」完全一致的物理网卡列表，自动过滤虚拟适配器 |
| **数据导出** | 监测结束自动生成单个 `.xlsx` 文件，一个 Sheet 一个进程，无外部库依赖 |
| **配置持久化** | JSON 配置文件（UTF-8 BOM），保存监测项、进程列表、采样周期、网卡选择、输出目录 |
...
│   ├── ProcessMonitor.h/cpp    # 进程级资源采集（~210 行）
│   │   ├── FindProcessPid      # CreateToolhelp32Snapshot 搜索 PID
│   │   ├── GetProcessCpuUsage  # GetProcessTimes 差分算法
│   │   └── GetProcessMemory    # GetProcessMemoryInfo
...
| `GetProcessMemory()` | `GetProcessMemoryInfo()` | 工作集大小，MB 精度 + 占系统百分比 |
| `Collect()` | 以上全部 | 含 PID 存活校验 (`QueryFullProcessImageNameW`) |

> **已知局限**：Windows 不暴露进程级网络流量（需 ETW 跟踪），进程网络列始终为 0。
...
**A**：这是 Windows 的已知限制。Windows 不提供进程级网络流量 API（需要 ETW 事件跟踪），进程网络列**始终为 0**。系统网络总流量正常即可。

### Q4：导出的 Excel 文件打不开或乱码
