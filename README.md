...
| **系统网络监测** | 发送 / 接收速度，支持 Kbps、Mbps、Gbps 三种单位，`GetIfTable2()` 64 位计数器 |
| **进程资源监测** | 按进程名添加，独立 Tab 页展示 PID / CPU / 内存 / 网络。同名多进程全部独立记录。内存使用「专用工作集」（与任务管理器一致） |
| **进程网络监测** | 基于 TCP 连接统计 API（`GetPerTcpConnectionEStats`），直接读取每个连接的字节计数器，非 ETW 方案。需管理员权限 |
| **网卡选择** | 与「控制面板 → 网络连接」完全一致的物理网卡列表，自动过滤虚拟适配器 |
| **数据导出** | 监测中每 2 秒实时写入 Excel（防崩溃丢失），停止时最终保存；一个 Sheet 一个进程，单元格居中对齐，列宽自适应 |
| **配置持久化** | JSON 配置文件（UTF-8），保存监测项、进程列表（含启用状态）、采样周期、网卡选择、输出目录 |
| **窗口置顶** | 右下角切换按钮，文字在「置顶」↔「取消置顶」间切换 |
| **彩色状态** | 监测控制组下方粗体状态标签：「就绪」绿色 / 「监测中」红色 / 「保存中」橙色 |
...
| **数据容量** | 内存缓冲区 200 万行（~115 天 @5s），ListView 显示窗口 5 万行；清除日志仅清空显示，不影响导出 |
...
│   ├── NetSpeedMonitor.h/cpp    # TCP连接统计 进程级网络流量
│   │   ├── GetExtendedTcpTable  # 枚举TCP连接（按PID过滤）
│   │   ├── Per-TCP-estats       # GetPerTcpConnectionEStats 字节计数器
│   │   └── Per-PID 聚合         # 两次采样增量 = 实时网速
│   ├── DataBuffer.h/cpp        # 线程安全环形缓冲区
│   │   ├── CRITICAL_SECTION    # 互斥锁保护
│   │   └── MAX_ROWS = 2000000  # ~115天@5s，安全上限自动裁剪
...
├──────────────────────────┴───────────────────────────────┤
│                    DataBuffer                             │
│       CRITICAL_SECTION 保护 · 环形缓冲 200 万行 · 显示窗口 5 万行   │
...
单位转换：Kbps = bytes/s × 8 / 1000  |  Mbps = bytes/s × 8 / 1000000  |  Gbps = bytes/s × 8 / 1000000000
```
...
| 阶段 | 行为 |
|------|------|
| 监测开始 | `BeginExport()` 创建文件并锁定（`FILE_SHARE_READ`），文件名 `monitor_data_开始时间.xlsx` |
| 监测中 | 每 2 秒 `FlushExport()` 重写全部数据到同一文件，外部可只读打开查看 |
| 停止监测 | `FlushExport()` 最终写入 + `EndExport()` 关闭句柄，弹出保存路径提示 |
| 崩溃恢复 | 最后一次成功写入的数据保留在磁盘上，不会全部丢失 |

**日志显示规则**：

| 组件 | 容量 | 裁剪策略 |
|------|------|---------|
| DataBuffer（内存） | 200 万行 | 环形缓冲，超过上限丢弃最旧数据 |
| ListView（界面） | 5 万行 | 超过上限裁剪最旧行（仅显示），数据仍完整保留 |
| 右键「清除日志」 | — | 仅清空 ListView 行，DataBuffer 和 Excel 不受影响 |
...
| `netUnit` | string | 网络单位：`Kbps` / `Mbps` / `Gbps` |
