...
| **数据安全** | 监测中每 10~30 秒增量写入 Excel，即使崩溃也不丢失已保存数据 |
| **低资源占用** | 监测线程使用 `THREAD_PRIORITY_BELOW_NORMAL`，UI 200ms 定时刷新，导出后主动释放内存 |
...
| ID | 功能 | 优先级 | 说明 |
|----|------|--------|------|
| FR-EXPORT-01 | 自动导出 Excel (.xlsx) | P0 | 监测中每 10~30 秒增量写入（自适应间隔），停止时最终保存 |
| FR-EXPORT-02 | 自动导出 HTML 报告 | P1 | 每 5 分钟自动生成 + 停止时最终生成，Canvas 2D 自绘折线图，支持 X 轴框选缩放、点击显示坐标值、图表过滤 |
...
| 数据缓冲区上限 | 内存 ~15min 热数据 + 磁盘临时文件（每 5 万行 _PartN） |
| Excel 实时刷新 | 每 10 秒增量刷新 |
| HTML 报告刷新 | 每 2 分钟自动生成 + 停止时最终生成 |
| 容量压缩 | 每 2 分钟 shrink_to_fit 释放容量膨胀 |
| 监测线程优先级 | `THREAD_PRIORITY_BELOW_NORMAL` |
| 自身 CPU 占用 | < 1%（@5s 采样周期） |
| 自身内存占用 | ~8 MB（1s 采样+100 PID），长期运行稳定 |
...
| 长时间运行 | 环形缓冲区防内存无限增长，导出后 `shrink_to_fit` 归还内存，ListView 裁剪防 UI 卡顿 |
| 稳定退出 | 监测线程超时强制终止 + 关闭 Timer 防竞态 + 跨翻译单元地址一致性修复，确保完全退出不留残留进程 |
...
| 缓冲区上限 50 万行 | 超过轮转 _PartN 文件（~29天@5s） | 可调节 `MAX_ROWS` |
...
| **V3.4.2** | 2026-07-21 | 内存优化（DataBuffer 增量切片、NetSpeedMonitor 全量 GC、缓冲区 50 万行上限）、Excel flush 间隔延长 + shrink_to_fit、文件轮转 _PartN（超 50 万行自动新建不丢数据）、HTML 报告显示监测时长、帮助/PRD/README 文档更新 |
| **V3.4.3** | 2026-07-21 | 内存泄漏修复（HTML 导出/ChartData/DataBuffer 全链路 `shrink_to_fit`）、进程退出稳定性修复（线程超时强制终止 + 关闭 Timer 防竞态 + `g_emptyProcess` ODR 修复）、MonitorThreadProc 停止检查点、移除 StopMonitoring 阻塞式 MessageBox、Excel 最终导出 + 内存清理 |
