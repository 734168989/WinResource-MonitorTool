...
private:
    struct PidCpuState {
        ...
        bool cpuInitialized;
        HANDLE hProcess;   // cached handle, reopened only when PID reconnects
        double smoothSendMbps;  // per-PID smoothed network speed
        double smoothRecvMbps;
    };
    ...
    std::vector<PidCpuState> m_pidStates;  // per-PID CPU baseline + smoothed net speed
