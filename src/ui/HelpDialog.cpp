...
static const wchar_t* g_helpContents[] = {
    ...
    L"   ■ Excel 自动保存\r\n"
    L"     • 监测中每 10~30 秒增量刷新 .xlsx 文件（自适应间隔）\r\n"
    ...
    L"     • 列宽自适应、宋体 11 号字体\r\n"
    L"   ■ 文件轮转\r\n"
    L"     • 数据超过 50 万行时自动创建新文件（_Part1, _Part2...）\r\n"
    L"     • 旧文件已保存不会丢失，新文件从空缓冲区继续采集\r\n"
    L"     • 保证长时间监测数据完整，不丢弃任何记录\r\n"
    L"   ■ 数据容量\r\n"
    L"     • 磁盘临时文件超 5 万行自动分割为 _PartN，防单文件过大\r\n"
    L"     • 内存中保留近 15 分钟热数据，完整历史在磁盘上\r\n"
    L"\r\n"
    L"6. HTML 趋势报告（Canvas 折线图可视化）\r\n"
    L"   • 监测中每 5 分钟自动刷新 + 停止时最终生成一份完整报告\r\n"
    ...
    L"\r\n"
    L"步骤 7 — 停止与导出\r\n"
    L"  点击\"停止监测\"按钮，数据最终保存为 Excel 文件和 HTML\r\n"
    L"  趋势报告。监测过程中数据已增量写入文件，可在输出目录以\r\n"
    L"  只读方式打开查看最新数据。\r\n"
    L"\r\n"
    L"  数据保存规则：\r\n"
    L"  • 监测中：每 10 秒增量刷新 Excel（防崩溃丢失）\r\n"
    L"  • 内存中保留近 15 分钟数据，完整历史写入磁盘临时文件\r\n"
    L"  • 停止时：从磁盘加载全量数据，最终保存 Excel + HTML 报告\r\n"
    L"  • 超 5 万行自动创建 _PartN 流水文件，防单文件过大\r\n"
    ...
    L"   ■ Excel 实时保存\r\n"
    L"     • 监测开始即创建 Excel 文件，每 10 秒增量刷新写入\r\n"
    ...
    L"   ■ 数据容量\r\n"
    L"     • 内存缓冲区上限 50 万行（约 29 天 @5s 采样）\r\n"
    L"     • 超过上限自动轮转 _PartN 文件，旧数据不丢失\r\n"
    ...
    L"   闭会弹出确认对话框，确认后自动保存数据并退出。退出逻辑已\r\n"
    L"   加固：线程超时强制终止 + 关闭 Timer 防竞态 + ODR 修复，确\r\n"
    L"   保任务管理器无残留进程。\r\n"
    L"\r\n"
    L"7. 权限说明\r\n"
